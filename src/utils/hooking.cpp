...
#pragma optimize("gt", on)
		DWORD find_pattern_in_module(const HMODULE module, const std::string_view& signature, DWORD offset, [[maybe_unused]] const char* description)
		{
			...
			if (description) {
				log("Hook", std::format("Could not find pattern '{}' ({})", signature, description), LOG_TYPE::LOG_TYPE_ERROR, true);
			} else {
				log("Hook", std::format("Could not find pattern '{}'", signature), LOG_TYPE::LOG_TYPE_ERROR, true);
			}
			return 0;
		}
		...
		DWORD find_pattern(module_info& module_info, const std::string_view& signature, const DWORD& offset, const char* description, const bool is_active, const DWORD& inactive_offset)
		{
			...
			for (size_t i = 0; i <= module_info.size - pattern_length; ++i)
			{
				...
				if (found) 
				{
					static bool validate_patterns = flags::has_flag("validate_patterns");
					const auto place = reinterpret_cast<DWORD>(base + i + offset);
					{
#if DEBUG
						if (description) {
						...
						if (validate_patterns) 
						{
							if (place == module_info.handle + inactive_offset + offset) {
								...
								log("Hook", std::format("> Pattern offset invalid!", place), LOG_TYPE::LOG_TYPE_ERROR, true);
							}
						}
					}
					return place;
...
#pragma optimize("", on)
		...
		uint32_t resolve_relative_jump_address(uint32_t instruction_addr, uint32_t instruction_size, uint32_t bytes_until_relative_addr)
		{
			...
			{
				log("Hook", std::format("Invalid offset size while resolving relative jump address @ (0x{:X})", instruction_addr), LOG_TYPE::LOG_TYPE_ERROR, true);
				return 0;
			}
	...
	bool hook::conditional_jump_to_jmp(const DWORD place)
	{
		...
		else if (short_jump_opcodes.contains(code[0])) 
		{
			...
			{
				log("Hook", std::format("CondJmpToJmp: Target address @ (0x{:X}) is out of range for a short JMP @ (0x{:X})", target_address, place), LOG_TYPE::LOG_TYPE_ERROR, true);
				return false;
			}
		...
		{
			log("Hook", std::format("CondJmpToJmp: Instruction @ (0x{:X}) is not a supported conditional jump (opcode: {:X})", place, first_two_bytes), LOG_TYPE::LOG_TYPE_ERROR, true);
			return false;
		}
		...
		{
			log("Hook", std::format("CondJmpToJmp: Failed to change memory protection @ (0x{:X})", place), LOG_TYPE::LOG_TYPE_ERROR, true);
			return false;
		}
		...
		if (debug_prints) {
			log("Hook", std::format("Patched {} to JMP @ (0x{:X})", jump_name, place), LOG_TYPE::LOG_TYPE_DEFAULT, false);
		}
		return true;
	}
	...
	bool hook::detour(const DWORD& offset, void* stub, void** original)
	{
		if (const auto state = MH_CreateHook(reinterpret_cast<LPVOID>(offset), stub, original);
			state != MH_OK)
		{
			log("Hook", std::format("Failed to install detour @ (0x{:X})", offset), LOG_TYPE::LOG_TYPE_ERROR, true);
			return false;
		}
		...
		if (const auto state = MH_EnableHook(reinterpret_cast<LPVOID>(offset));
			state != MH_OK)
		{
			log("Hook", std::format("Failed to activate detour @ (0x{:X})", offset), LOG_TYPE::LOG_TYPE_ERROR, true);
			return false;
		}
